...
function enriquecerNotificacion(notificacion, contratosPorId) {
  const contrato = contratosPorId.get(notificacion.contrato_id);
  return {
    ...notificacion,
    contrato: contrato ? {
      id: contrato.id,
...
router.get('/notificaciones', authenticate, (req, res) => {
  ...
  const notificaciones = store
    ...
    .map(n => enriquecerNotificacion(n, contratosPorId));
...
router.patch('/notificaciones/:id/leer', authenticate, (req, res) => {
  ...
  if (!contratosDelUsuario(req.user).has(notif.contrato_id)) return res.status(403).json({ error: "No autorizado" });
